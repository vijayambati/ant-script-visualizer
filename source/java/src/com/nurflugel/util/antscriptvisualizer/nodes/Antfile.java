...
public class Antfile
{
    ...
    public Antfile(File                  antFile,
                   Map<String, Property> aproperties) throws IOException, JDOMException
    {
        buildFile  = antFile;
    ...
    public Antfile(Element              antcall,
                   EventCollector       eventCollector,
                   List<Antfile>        importsAlreadyProcessed,
                   List<Antfile>        importsToProcess,
    ...
    public String getDefaultTargetName()
    {
        ...
        if (defaultTargetAttribute != null) { return defaultTargetAttribute.getValue(); }
        else { return ""; }
    }
    ...
    public boolean equals(Object o)
    {
        ...
        if (!(o instanceof Antfile)) { return false; }
    ...
    @Override
    public int hashCode() { return buildFile.getAbsolutePath().hashCode(); }

    /**  */
    @Override
    public String toString() { return buildFile.getName(); }
    ...
    public List<AntCall> getAntCalls() { return antCalls; }
    ...
    public List<Antfile> getImports() { return imports; }

    /** Any macrodefs used in this build file. */
    public List<Macrodef> getLocalMacrodefs() { return localMacrodefs; }

    /** Any imports used in this build file. */
    public List<Taskdef> getLocalTaskdefs() { return localTaskdefs; }
     ...
     */
    public void parse(AntFileParser  parser,
                      List<Antfile>  importsToProcess,
                      List<Antfile>  importsAlreadyProcessed,
                      EventCollector eventCollector,
    ...
    private void parseImports(AntFileParser  parser,
                              List<Antfile>  importsToProcess,
                              List<Antfile>  importsAlreadyProcessed,
                              EventCollector eventCollector)
    {
    ...
    private void parseTargets(EventCollector eventCollector,
                              List<Antfile>  importsToProcess,
                              List<Antfile>  importsAlreadyProcessed,
                              AntParserUi    ui)
    {
        ...
        while (descendants.hasNext()) {
            ...
            String    description        = (descriptionElement != null)
                ? descriptionElement.getValue().trim()
                : "";
    ...
    public void parseForAntcalls()
    {
        ...
        for (Target calledFromTarget : targets) {
            ...
            for (Iterator targetsIterator = rootElement.getDescendants(new ElementFilter("calledFromTarget")); targetsIterator.hasNext();) {
                ...
                if (targetElement.getAttribute("name").getValue().equalsIgnoreCase(calledFromTarget.getName())) // parse this calledFromTarget for
                                                                                                                // usages of "ant"
                {
                    ...
                    for (Iterator antIterator = targetElement.getDescendants(new ElementFilter("ant")); antIterator.hasNext();) { // we found one!
                        ...
                        String  antfile    = buildFile.getAbsolutePath(); // default value if they don't specify an antfile, then THIS is the
    ...
    public List<Antfile> parseForAnts(EventCollector eventCollector) throws IOException, JDOMException
    {
        ...
        for (Target target : targets) {
            ...
            for (Iterator targetsIterator = rootElement.getDescendants(new ElementFilter("target")); targetsIterator.hasNext();) {
                ...
                if (targetElement.getAttribute("name").getValue().equalsIgnoreCase(target.getName())) // parse this target for usages of "ant"
                {
    ...
    private void parseAnt(Iterator       antIterator,
                          List<Antfile>  antfiles,
                          EventCollector eventCollector)
    {
    ...
    private Target getDefaultTarget()
    {
        ...
        if (defaultTargetAttribute != null) { return new Target(defaultTargetAttribute.getValue(), this); }
        else { return null; }
    }
    ...
    private Macrodef getMatchingMacrodef(Element targetElement)
    {
        ...
        for (Macrodef macrodef : localMacrodefs) {
            ...
            if (label.equals(targetName)) { return macrodef; }
        }
    ...
    private Target getMatchingTarget(Element targetElement)
    {
        ...
        for (Target target : targets) {
            ...
            if (label.equals(targetName)) { return target; }
        }
